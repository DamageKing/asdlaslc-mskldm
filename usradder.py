...
def banner():
    ...
    print(f'{info}{g} Telegram Üye Ekleyici v4.1{rs}')
    print(f'{info}{g} Yapımcı: ArdaBarut{rs}\n')
...
print(f'{info}{g} Üyeler Ekleniyor {group_name}{rs}\n')
n = 0
added_users = []
for user in users:
    n += 1
    ...
    try:
        ...
        usr_id = user['user_id']
        print(f'{attempt}{g} Eklendi {usr_id}{rs}')
        print(f'{sleep}{g} Bekleniyor 10s{rs}')
        time.sleep(10)
    except PeerFloodError:
        #time.sleep()
        os.system(f'del {file}')
        sys.exit(f'\n{error}{r} İptal edildi. Flood Hatası{rs}')
    except UserPrivacyRestrictedError:
        print(f'{error}{r} Kullanıcı Gizlilik Ayarı!{rs}')
        continue
    except KeyboardInterrupt:
        print(f'{error}{r} İptal edildi. Klavye Kesintisi{rs}')
        update_list(users, added_users)
        if not len(users) == 0:
            print(f'{info}{g} Kalan kullanıcılar oturum açtı {file}')
            logger = Relog(users, file)
            logger.start()
    except:
        print(f'{error}{r} eklerken başka bir hata{rs}')
        continue
#os.system(f'del {file}')
input(f'{info}{g}Ekleme tamam enter a basarak çıkın...')
sys.exit()
