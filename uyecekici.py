...
while True:
    try:
        ...
        f.close()
        break
print(f'{INPUT}{cy} Üyeleri çekmek için bir hesap seçin\n')
i = 0
for acc in accs:
...
api_hash = accs[ind][1]
phone = accs[ind][2]
group_name = input(f"@ olmadan grubun adını girin: {r}")
c = TelegramClient(f'sessions\\{phone}', api_id, api_hash)
c.connect()
if not c.is_user_authorized():
    try:
        c.send_code_request(phone)
        code = input(f'{INPUT}{lg} için oturum açma kodunu girin {w}{phone}{r}: ')
        c.sign_in(phone, code)
    except PhoneNumberBannedError:
        print(f'{error}{w}{phone}{r} banlanmış!{rs}')
        print(f'{error}{lg} Çalıştırın {w}menu.py{lg} to filter them{rs}')
        sys.exit()
group = c.get_entity(group_name)
target_grp = "t.me/" + group_name

choice = int(input(f"\n{lg}Kullanıcıları nasıl elde etmek istersiniz?\n\n{r}[{lg}0{r}]{cy} Tüm Kullanıcılar\n{r}[{lg}1{r}]{cy} Aktif kullanıcılar(bugün ve dün aktif olanlar)\n{r}[{lg}2{r}]{cy} Geçen hafta aktif olan kullanıcılar\n{r}[{lg}3{r}]{cy} Geçen ay aktif olan kullanıcılar\n{r}[{lg}4{r}]{cy} Aktif olmayan üyeler(geçen ay aktif olan üyeler) {lg}\n\nSeçiminiz: "))
members = []
...
admin_choice = input(f"{lg}Yöneticilerin ayrı bir CSV dosyasına alalımmı? {rs}[y/n] {lg}")
...
with open("members\\members.csv", "w", encoding='UTF-8') as f:
    ...
    if choice == 0:
        try:
            ...
            print("\nBir FloodWaitError vardı, ancak üyeler.csv'yi kontrol edin. Üyelerin 95%%'inden fazlası zaten eklenmiş olmalıdır.")
    elif choice == 1:
        try:
            for index,member in enumerate(members):
                print(f"{index+1}/{cont}", end="\r")
        ...
        try:
            ...
            print(f"\n{r}Bir FloodWaitError vardı, ancak üyeler.csv'yi kontrol edin. Üyelerin %95'inden fazlası zaten eklenmiş olmalıdır.")
                
f.close()

print(f"\n{lg}Kullanıcılar csv dosyasına kaydedildi.{rs}\n")
clr()
banner()
with open('zhedef.txt', 'w') as f:
    f.write(target_grp)
f.close()
sys.exit()
